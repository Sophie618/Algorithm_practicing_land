...
order* creat_order(int stock_id,double price,int quantity,char type){
...
void process_new_order(int stock_id,double price,int quantity,char type){
    order* new_order=create_order(stock_id,price,quantity,type);
    cout<<"orderid: "<<setw(4)<<setfill('0')<<endl;
...
void matchAndInsert(order* new_order,stockQueue* stock){
    ...
    while(new_order->quantity>0&&*oppositeHead!=nullptr){
        ...
        double dealPrice=(new_order->price+topOrder->price)/2;
        int dealQuantity=min(new_order->quantity,topOrder->quantity);

        if(isBuyOrder){
            printf("deal--price:%6.1f  quantity:%4d  sellorder:%04d  buyorder:%04d\n"
                  ,dealPrice, dealQuantity, topOrder->orderId, new_order->orderId);
        }
        else{
            printf("deal--price:%6.1f  quantity:%4d  sellorder:%04d  buyorder:%04d\n"
                  ,dealPrice, dealQuantity, new_order->orderId, topOrder->orderId);
        }
    ...
    if(newOrder->quantity>0){
        insertOrder(new_order,sameHead);
    }
    else{
        delete new_order;
    }
...
void insertOrder(order* new_order,order** head){
    if(*head==nullptr){
        *head=new_order;
        return;
    }
