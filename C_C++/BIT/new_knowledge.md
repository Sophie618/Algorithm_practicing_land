...
- 在C++中，'()' 是一个多字符常量 (multi-character literal)，它的值是实现定义的 (implementation-defined)。编译器会尝试将这两个字符 '(' 和 ')' 的ASCII码组合成一个 int 值。

- '(' 的 ASCII 码是 40 (十进制)。
- ')' 的 ASCII 码是 41 (十进制)。
- 在你的系统上，编译器很可能将它们解释为 (40 * 256) + 41 = 10240 + 41 = 10281。

- cout 接收到这个 int 值 10281，并将其作为数字打印出来。

### 结论与修复
- 最直接的错误：将 cout << '()'; 改为 cout << "()";。单引号用于单个字符，双引号用于字符串。
